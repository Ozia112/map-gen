...
def _compute_z_base(min_h: float, max_h: float) -> float:
    z_range_raw = float(max_h - min_h)
    extra = 0.20 * (z_range_raw if z_range_raw > 0 else 1.0)
    return float(min_h - extra)
...
def draw_map_3d(generator, num_contour_levels, elevation_angle, azimuth_angle, show_axis_labels=True, line_color='#ff7825', grid_color='#00ffff', grid_width=0.6, grid_opacity=0.35):
    ...
    # Calcular niveles de contorno de forma segura
    if max_h > min_h:
        ...
        # Perímetro superior siguiendo el terreno en los bordes
...
def export_map_clean(generator, visual_params, fmt='png', save_path=None, include_grid=None, scale=1):
    ...
    if max_h > min_h:
        ...
        # Soportes y caja con margen inferior
        corners = [
            (0, 0),
            (generator.width - 1, 0),
            (generator.width - 1, generator.height - 1),
        ...
        # Perímetro superior siguiendo el terreno
    rgba = mcolors.to_rgba(line_color, alpha=0.8)
    ...
    if max_h > min_h:
        ...
        for level in levels:
            temp_ax.contour(
                ...
                alpha=0.85, zdir='z', offset=level, zorder=5
            )
        # Caja con margen inferior y perímetro superior siguiendo el terreno
