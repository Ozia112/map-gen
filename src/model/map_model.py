...
class MapModel:
    ...
    def update_terrain_params(self, **kwargs) -> Dict[str, Any]:
        ...
        self.terrain_params.update(validated)
        return validated
    ...
    def update_visual_params(self, **kwargs) -> Dict[str, Any]:
        ...
        self.visual_params.update(validated)
        return validated
    ...
    def _validate_terrain_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'height_variation' in params:
            ...
            if not 0 <= vh <= 20:
                raise ValueError(f"height_variation debe estar entre 0 y 20, recibido: {vh}")
            validated['height_variation'] = vh

        if 'terrain_roughness' in params:
            roughness = int(params['terrain_roughness'])
            if not 0 <= roughness <= 100:
                raise ValueError(f"terrain_roughness debe estar entre 0 y 100, recibido: {roughness}")
    ...
    def _validate_visual_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'line_color' in params:
            ...
            if not isinstance(color, str) or not color.startswith('#'):
                raise ValueError(f"line_color debe ser un string hexadecimal, recibido: {color}")
        ...
        if 'elevation_angle' in params:
            ...
            if not 0 <= el <= 90:
                raise ValueError(f"elevation_angle debe estar entre 0 y 90, recibido: {el}")
