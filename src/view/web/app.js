...
const state = { terrain: {}, visual: {}, craters: {} };

function applyState(s, updateSliders = true) {
  Object.assign(state.terrain, s.terrain);
  Object.assign(state.visual, s.visual);
  // El backend envía 'craters' (o puede ser 'crater'), normalizar
  const craters = s.craters || s.crater || {};
  Object.assign(state.craters, craters);
  
  // DEBUG: Ver qué llega
  console.log('applyState recibió:', s);
  console.log('terrain_stats:', s.terrain_stats);
  
  // Actualizar máximo del slider de nivel del mar basado en altura máxima del terreno
  if (s.terrain_stats && s.terrain_stats.max_height) {
    ...
    if (parseFloat(els.seaLevel.value) > newMax) {
      ...
      els.seaLevelVal.textContent = newMax.toFixed(1);  // Actualizar display
    }
  }
  
  // Solo actualizar valores de sliders si se solicita (evita loops)
  ...
  // Cráteres - usar nombres del backend: enabled
  const cratersEnabled = state.craters.enabled ?? false;
  els.chkCraters.checked = cratersEnabled;
  // Sliders - usar nombres normalizados: height_variation, terrain_roughness y base_height
  ...
  // Cráteres - usar nombres del backend: density, size, depth
...
function previewUpdate() {
  ...
  eel.api_update(params)().then((res) => {
    if (res && res.ok) {
      // Actualizar terrain_stats sin tocar los sliders (evita loops)
...
function wire() {
  ...
  els.btnSeedRandom.onclick = ()=> {
    ...
    eel.api_random_seed()().then((r)=>{ 
      showLoader(false);
      if (r && r.ok) {
        // Obtener el estado actualizado del servidor
  ...
  // Inicializar state.craters si no existe
  if (!state.craters) state.craters = {};
  
  els.chkCraters.onchange = ()=> { 
    state.craters.enabled = !!els.chkCraters.checked; 
    els.cratersSection.hidden = !state.craters.enabled; 
    previewUpdate(); 
  };

  // Usar nombres normalizados: height_variation, terrain_roughness y base_height
  els.vh.oninput = ()=> { 
    state.terrain.height_variation = parseFloat(els.vh.value); 
    els.vhVal.textContent = state.terrain.height_variation.toFixed(1); 
    previewUpdate(); 
  };
  els.rough.oninput = ()=> { 
    state.terrain.terrain_roughness = parseInt(els.rough.value); 
    els.roughVal.textContent = `${state.terrain.terrain_roughness}%`; 
    previewUpdate(); 
  };
  els.baseHeight.oninput = ()=> {
    state.terrain.base_height = parseFloat(els.baseHeight.value);
    els.baseHeightVal.textContent = state.terrain.base_height.toFixed(1);
    previewUpdate();
  };
  els.seaLevel.oninput = ()=> {
    state.visual.sea_level = parseFloat(els.seaLevel.value);
    els.seaLevelVal.textContent = state.visual.sea_level.toFixed(1);
    previewUpdate();
  };
  els.dens.oninput = ()=> { 
    state.visual.num_contour_levels = parseInt(els.dens.value); 
    els.densVal.textContent = state.visual.num_contour_levels; 
    previewUpdate(); 
  };
  // Rotación: AZ
  ...
  els.azRange.oninput = ()=> { 
    const v = clamp(parseInt(els.azRange.value),0,360); 
    els.azInput.value = v; 
    state.visual.azimuth_angle = v; 
    previewUpdate(); 
  };
  els.azInput.onchange = ()=> { 
    let v = parseInt(els.azInput.value); 
    if(isNaN(v)) v = state.visual.azimuth_angle; 
    v = clamp(v,0,360); 
    els.azInput.value=v; 
    els.azRange.value=v; 
    state.visual.azimuth_angle=v; 
  ...
  // Rotación: EL
  els.elRange.oninput = ()=> { 
    const v = clamp(parseInt(els.elRange.value),0,90); 
    els.elInput.value = v; 
    state.visual.elevation_angle = v; 
    previewUpdate(); 
  };
  els.elInput.onchange = ()=> { 
    let v = parseInt(els.elInput.value); 
    if(isNaN(v)) v = state.visual.elevation_angle; 
    v = clamp(v,0,90); 
    els.elInput.value=v; 
    els.elRange.value=v; 
    state.visual.elevation_angle=v; 
  ...
  // Cráteres - usar nombres correctos del backend: density, size, depth
  els.cDen.oninput = ()=> { 
    state.craters.density = parseInt(els.cDen.value); 
    els.cDenVal.textContent = state.craters.density; 
    previewUpdate(); 
  };
  els.cSize.oninput = ()=> { 
    state.craters.size = parseFloat(els.cSize.value); 
    els.cSizeVal.textContent = state.craters.size.toFixed(1); 
    previewUpdate(); 
  };
  els.cDepth.oninput = ()=> { 
    state.craters.depth = parseFloat(els.cDepth.value); 
    els.cDepthVal.textContent = state.craters.depth.toFixed(1); 
    previewUpdate(); 
  };
